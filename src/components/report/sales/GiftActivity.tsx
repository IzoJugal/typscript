...
const GiftActivity = () => {
    ...
    const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
        ...
        setFormData(prev => ({
            ...prev,
            [name]: type === 'checkbox' ? (e.target as HTMLInputElement).checked : (name === 'isVoid' ? value === 'true' : value), // Convert string to boolean for isVoid
        }));
    ...
    return (
        <div>
            <div>
                <FormHeaderPaths page={'Gift Cert. Activity Report'} prevLink='#' prevPage='Sales' />
            </div>
            <div className="relative max-w-2xl mx-auto p-4 bg-white dark:bg-DARK-800 shadow-md rounded-lg">
                <h2 className="text-2xl font-bold mb-6 text-center dark:text-white">Gift Cert. Activity Report</h2>
                <form className="space-y-6">
                    <div className="space-y-6">
                        <div className="flex flex-col">
                            <Label className="block text-sm font-medium text-DARK-700 mb-1">Gift Cert #</Label>
                            <input
                                type="text"
                                name="giftCart"
                                value={formData.giftCart}
                                onChange={handleChange}
                                placeholder="Enter gift certificate number"
                                className="w-full px-3 py-2 dark:bg-DARK-700 dark:placeholder:text-DARK-400 dark:text-DARK-200  dark:border-none border bg-DARK-100 rounded-md"
                            />
                        </div>
                        <div>
                    ...
                    <div>
                        <div className="mb-4 flex items-center justify-start space-x-4">
                            ...
                            <Button className="flex gap-1 justify-center items-center bg-BRAND-500 hover:!bg-BRAND-600 dark:bg-BRAND-500" onClick={() => { handleSubmit("print") }}>
            ...
            <Modal show={openModal} onClose={() => setOpenModal(false)} className="backdrop-blur-sm dark:bg-DARK-950">
                <Modal.Header>Gift Certificate Activity Report</Modal.Header>
                <Modal.Body>
