...
const ReservationModal = ({ openModal, setOpenModal, selectedObject, onSubmit, rooms, setSelectedRoom, setSelectedObject, loginRole, selectedCompany, selectedRestaurant }: any) => {
  ...
  const isValid = () => {
    ...
    if (!formData.guests) {
      isValid = false;
      errors.guests = "Guests are required";
    }
  ...
  const checkTablesInOtherRooms = (e: any) => {
    ...
    setAvailableTables([]);
    availableTablesAPI(NewFormData);
  };
  ...
  const handlePackage = (value: string) => {
    setFormData((prev:any) => ({ ...prev, package: value }));
  };
    ...
    <>
      <Modal show={openModal} onClose={handleCancel} className="backdrop-blur-sm dark:bg-DARK-950">
        <Modal.Header className="bg-BRAND-100 dark:bg-DARK-800 text-white">
          Book reservation in the {selectedObject?.room?.name} room
        </Modal.Header>
        <Modal.Body className="pt-1 dark:bg-DARK-800">
          <div className="flex justify-between items-center mb-4 text-DARK-700 dark:text-DARK-300 font-semibold text-md">
            <span className="text-sm">Available Tables in {selectedObject?.room?.name} room
              {formData.guests &&
                <span className="text-sm mx-1">for {formData?.guests} members </span>
              }
            </span>
              ...
              {formData.tableIds.length > 0 && availableTables.reduce((acc: any, table: any) => {
                ...
                return acc;
                  ...
                  <div className="text-yellow-400 font-semibold text-sm mt-2">
                    Warning: You have selected tables with more capacity than the number of guests.
                  </div>
                )}
