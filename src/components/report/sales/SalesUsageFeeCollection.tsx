...
const SalesUsageFeeCollection = () => {
    ...
    const handlePreview = async () => {
        try {
            ...
            const response = await apiClient.get(`/category/sales/usage-fee-collection${params}`, {
                responseType: "blob",
            });
    ...
    return (
        <div>
            <div>
                {/* Heading */}
                <FormHeaderPaths page={'Usage Fee Collection'} prevLink='#' prevPage='Sales' />
            </div>
            <div className="relative max-w-2xl mx-auto p-4 bg-white dark:bg-DARK-800 shadow-md rounded-lg">
                <h2 className="text-2xl font-bold mb-6 text-center dark:text-white">Usage Fee Collection</h2>
                <form className="space-y-6">
                    <div className="space-y-4">
                        ...
                        <div className="flex-1 pb-4">
                            <label htmlFor="fromDate" className="block text-sm font-medium text-DARK-700 mb-1 dark:text-DARK-200">Group By</label>
                            <select
                                name="type"
                                value={formData?.type}
                            ...
                            >
                                ...
                                <option value="posGroup">Pos Screen Group</option>
                            </select>
                            {errors.fromDate && <p className="mt-1 text-sm text-ERROR_HOVER">{errors.fromDate}</p>}
                        </div>
                    </div>
                    <div>
            ...
            <Modal size="7xl" show={openModal} onClose={() => setOpenModal(false)} className="backdrop-blur-sm dark:bg-DARK-950">
                <Modal.Header>Usage Fee Collection Report</Modal.Header>
                <Modal.Body>
...
export default SalesUsageFeeCollection