...
const AutoTips = () => {
  ...
  const handlePreview = async () => {
    try {
      ...
      const response = await apiClient.get(`/category/sales/tip${params}`, {
        responseType: "blob",
      });
  ...
  return (
    <div>
      <div>
        {/* Heading */}
        <FormHeaderPaths page={'Auto Tips'} prevLink='#' prevPage='Sales' />
      </div>
      <div className="relative max-w-2xl mx-auto p-4 bg-white dark:bg-DARK-800 shadow-md rounded-lg">
        <h2 className="text-2xl font-bold mb-6 text-center dark:text-white">Auto Tips </h2>
      ...
      <Modal size="7xl" show={openModal} onClose={() => setOpenModal(false)} className="backdrop-blur-sm dark:bg-DARK-950">
        <Modal.Header>Tip Report</Modal.Header>
        <Modal.Body>
