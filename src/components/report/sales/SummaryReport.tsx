...
const SummaryReport = () => {
  ...
  const handlePreview = async () => {
    try {
      ...
      const response = await apiClient.get(`/category/sales/summary${params}`, {
        responseType: "blob",
      });
  ...
  return (
    <div>
      <div>
        {/* Heading */}
        <FormHeaderPaths page={'Sales Summary Report'} prevLink='#' prevPage='Sales' />
      </div>
      <div className="relative max-w-2xl mx-auto p-4 bg-white dark:bg-DARK-800 shadow-md rounded-lg">
        <h2 className="text-2xl font-bold mb-6 text-center dark:text-white">Sales Summary Report</h2>
        <form className="space-y-4">
          ...
          {OWNER_ROLES.includes(loginRole) && (<div className="col-span-1">
            ...
            <DropdownWithSearch
              setSelectedItem={setFormData}
              selectedItem={restaurant?.find((r: any) => r._id === formData?.restaurant)?.name || ''}
              items={restaurant}
              title="Restaurant"
              setIsDropdownOpen={setIsDropdownOpen}
              isDropdownOpen={isDropdownOpen}
      ...
      <Modal size="7xl" show={openModal} onClose={() => setOpenModal(false)} className="backdrop-blur-sm dark:bg-DARK-950">
        <Modal.Header>Sales Summary Report</Modal.Header>
        <Modal.Body>
