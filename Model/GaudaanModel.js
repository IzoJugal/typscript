...
const gaudaanSchema = new mongoose.Schema(
  {
    ...
    animalType: { type: String, enum: ['cow', 'buffalo', 'other'], required: true },
    animalCondition: { type: String, enum: ['healthy', 'sick', 'injured'], default: 'healthy' },
    ...
    status: {
      type: String,
      enum: ['unassigned', 'assigned', 'picked_up', 'shelter', 'dropped', 'rejected'],
      default: 'unassigned',
    },
    statusHistory: [
      {
        status: { type: String, enum: ['unassigned', 'assigned', 'picked_up', 'shelter', 'dropped', 'rejected'] },
