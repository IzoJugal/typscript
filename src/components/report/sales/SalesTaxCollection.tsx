...
const SalesTaxCollection = () => {
  ...
  const handlePreview = async () => {
    try {
      ...
      const response = await apiClient.get(`/tax/sales/report${params}`, {
        responseType: 'blob',
      });
      ...
      const errorMessage = error?.response?.data?.message || "tax data not found";
  ...
  return (
    <div>
      <div>
        {/* Heading */}
        <FormHeaderPaths page={'Sales Tax Collection Report'} prevLink='#' prevPage='Sales' />
      </div>
      <div className="relative max-w-2xl mx-auto p-4 bg-white dark:bg-DARK-800 shadow-md rounded-lg">
        <h2 className="text-2xl font-bold mb-6 text-center dark:text-white">Sales Tax Collection Report</h2>
        <div>
        ...
        <div>
          <div className="mb-4 py-3 flex items-center justify-start space-x-4">
      ...
      <Modal size="7xl" show={openModal} onClose={() => setOpenModal(false)} className="backdrop-blur-sm dark:bg-DARK-950">
        <Modal.Header>Tax Detail Report</Modal.Header>
        <Modal.Body>
...
export default SalesTaxCollection;